...
const doLikeReply = (replyId: number) => {
  return axiosInstance.post(`/private/comments/replies/${replyId}/like`);
};

// 커뮤니티 답글 좋아요 취소하기 API
const unDoLikeReply = (replyId: number) => {
  return axiosInstance.delete(`/private/comments/replies/${replyId}/like`);
};
...
export default function ReplyListItem(props: ReplyListItemProps) {
  ...
  const doLikeReplyMutation = useMutation({
    mutationFn: doLikeReply,
    onMutate: () => {},
  ...
  const unDoLikeReplyMutation = useMutation({
    mutationFn: unDoLikeReply,
    onMutate: () => {},
    onError: (error: AxiosError) => {
      const resData: any = error.response;
    ...
    <div className='flex gap-x-1 px-1'>
      ...
      <div className='w-full flex flex-col gap-y-2'>
        ...
        <div className='flex items-center gap-x-2'>
          <button
            onClick={() => {
              if (replyInfo.isLiked) {
                unDoLikeReplyMutation.mutate(replyInfo.replyId);
                return;
              }

              doLikeReplyMutation.mutate(replyInfo.replyId);
            }}
            className={`relative ml-[-0.25rem] flex items-center bg-[#f2f4f6] text-[#4e5968] rounded-full px-2 py-[0.3rem] ${
              replyInfo.isLiked && 'outline outline-1 outline-[#3a8af9]'
            }`}
          >
            ...
            <span
              className={`ml-[1.375rem] ${
                replyInfo.isLiked ? 'text-gray-600' : 'text-[#8b95a1]'
              } text-xs`}
            >
              {replyInfo.likeCount}
            </span>
