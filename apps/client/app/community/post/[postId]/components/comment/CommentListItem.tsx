        ...
        <button
          onClick={() => {
            if (commentInfo.isLiked) {
              unDoLikeCommentMutation.mutate(commentInfo.commentId);
              return;
            }

            doLikeCommentMutation.mutate(commentInfo.commentId);
          }}
          className={`relative ml-[-0.25rem] flex items-center bg-[#f2f4f6] text-[#4e5968] rounded-full px-2 py-[0.3rem] ${
            commentInfo.isLiked && 'outline outline-1 outline-[#3a8af9]'
          }`}
        >
          ...
          <span
            className={`ml-[1.375rem] ${
              commentInfo.isLiked ? 'text-gray-600' : 'text-[#8b95a1]'
            } text-xs`}
          >
